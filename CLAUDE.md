...
- [sumo_data.py](sumo_data.py) — Pure data layer. No network, no formatting, no side effects. Owns the reference tables — `HONBASHO_DATA` (per-month: name, city, venue, image URL, embed color), `RANK_ANSI` (rank initial → ANSI fg/style, shared by every builder that colors a line by rank), `SPECIAL_PRIZES` (romanized prize name → English name + what it's awarded for) — the ANSI helpers (`ansi`, `ansi_block`), helper functions (`parse_short_rank`, `get_form_string`, `get_h2h_wins`, `get_previous_basho`, `get_current_basho_id`, `get_current_day`), and the data classes that summarize an API response into a builder-friendly shape: `BashoData`, `SanyakuData`, `StakesData`, `SummaryData`, plus `build_rikishi_stats`.
